...
function App() {
  ...
  const archetypes = [
    { id: 'epic', label: 'Epic', icon: '­ƒÅø´©Å' },
    { id: 'gritty', label: 'Gritty', icon: '­ƒöÑ' },
    { id: 'fast', label: 'Fast', icon: 'ÔÜí' },
    { id: 'deep', label: 'Deep', icon: '­ƒîè' },
  ];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-6 max-w-md mx-auto pb-32 selection:bg-amber-500/30">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-md w-full shadow-2xl">
            <div className="text-center mb-6">
              <div className="text-6xl mb-4">­ƒÄº</div>
            ...
            <div className="bg-slate-800/50 rounded-2xl p-4 mb-6">
              <h4 className="text-amber-400 font-semibold mb-2">­ƒÆí Pro Tips</h4>
              <ul className="text-slate-300 text-sm space-y-2">
                <li>ÔÇó Try different archetypes for varied recommendations</li>
                <li>ÔÇó Click history items to revisit past finds</li>
                <li>ÔÇó Higher match scores mean better compatibility</li>
                <li>ÔÇó Add your Amazon affiliate tag to earn money!</li>
              </ul>
            </div>
      ...
      {recommendation ? (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <p className="text-slate-400 text-sm mb-6">by {recommendation.author} ÔÇó <span className="text-amber-500 font-medium">{recommendation.narrator}</span></p>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-slate-950/80 backdrop-blur-xl border-t border-slate-900 p-6 text-center z-50">
        <p className="text-[9px] text-slate-800 font-bold uppercase tracking-[0.5em]">VIBEARY SYSTEMS ÔÇó 2026</p>
      </div>
    </div>
  );
}
